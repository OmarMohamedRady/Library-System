...
public class PatronService {
    ...
    List<Patron> findAll(Patron patron){
        if(patron == null || patron.getStart() == null || patron.getLimit() == null || (patron.getStart() == 0 && patron.getPatronID() == 0)){
    ...
    public void deleteById(Long patronId) {
        ...
        List<BorrowingRecord> borrowingRecordList = borrowingRecordRepository.findByPatronIdAndReturnDateIsNotNull(patronId);
        if(!borrowingRecordList.isEmpty()){
            borrowingRecordRepository.deleteAll(borrowingRecordList);
        }else {
           borrowingRecordList = borrowingRecordRepository.findByPatronIdAndReturnDateIsNull(patronId);
           if(!borrowingRecordList.isEmpty()){
